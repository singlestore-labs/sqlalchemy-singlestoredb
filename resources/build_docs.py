#!/usr/bin/env python3
"""
Unified documentation build script for SQLAlchemy SingleStoreDB.
...
def get_repo_paths() -> tuple[Path, Path]:
    ...
    elif (script_dir / 'sqlalchemy_singlestoredb').exists():
        # We're in the repo root
        repo_root = script_dir
        docs_src = repo_root / 'docs' / 'src'
    else:
        # Try to find the repo root by looking for sqlalchemy_singlestoredb directory
        current = script_dir
        while current != current.parent:
            if (current / 'sqlalchemy_singlestoredb').exists():
                repo_root = current
                docs_src = repo_root / 'docs' / 'src'
                break
...
# Try to import Docker utilities from singlestoredb package
try:
...
class DocBuilder:
    ...
    def start_docker_container(self) -> bool:
        ...
        if not DOCKER_AVAILABLE:
            self.status('⚠️  singlestoredb.server.docker not available, skipping container start')
            self.status('   (Install singlestoredb package or set SINGLESTOREDB_URL to use a database)')
            return True
    ...
    def apply_content_transformations(self, content: str, links: Dict[str, str]) -> str:
        ...
        content = re.sub(
            r'(">)sqlalchemy_singlestoredb\.(base)\.([\w\.]+</a>)',
            r'\1\3',
            content,
        )

        # Change ShowAccessor to Connection.show (if applicable)
        content = re.sub(r'>ShowAccessor\.', r'>Connection.show.', content)

        # Remove paramrefs
        content = re.sub(r':paramref:', r'', content)
    ...
    def build_docs(self) -> int:
        ...
        try:
            ...
            print(f'🚀 Starting SQLAlchemy SingleStoreDB documentation build ({self.target})', file=sys.stderr)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description='Build SQLAlchemy SingleStoreDB documentation',
